...
module.exports = class Proyecto{
    ...
    static fetchProyectosUsuario(nombre_usuario){
        return db.execute('SELECT P.nombre_proyecto, descripcion, imagen FROM proyecto_usuario AS PU,proyecto AS P WHERE PU.nombre_proyecto = P.nombre_proyecto AND nombre_usuario = ? ORDER BY fecha DESC', [nombre_usuario]);
    }

    static fetchProyectoUsuarioByName(nombre_proyecto, nombre_usuario){
        return db.execute('SELECT P.nombre_proyecto, descripcion, imagen FROM proyecto_usuario AS PU,proyecto AS P WHERE PU.nombre_proyecto = P.nombre_proyecto AND P.nombre_proyecto LIKE ? AND nombre_usuario = ? ORDER BY fecha DESC', ['%'+nombre_proyecto+'%', nombre_usuario]);
    }
} 