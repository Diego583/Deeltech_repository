function horario(nombre_usuario, proyecto_id) {
    const horario = document.getElementById('s' + nombre_usuario).value;
    //El token de protección CSRF
    const csrf = document.getElementById('_csrf').value;
    
    const id = document.getElementById(nombre_usuario).value;

    console.log("id:" + id);
    console.log("horario:" + horario);

    let data = {horario: horario, id: id};
    //console.log(valor_busqueda);
    //función que manda la petición asíncrona
    fetch('/proyectos/' + proyecto_id + '/horario', {
        method: 'POST',
